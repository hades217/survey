...
(function() {
    ...
    function universalBlock(e) {
        ...
        if (['copy', 'cut', 'paste'].includes(type)) {
            ...
            e.stopPropagation();
            e.stopImmediatePropagation();
            alert(`⚠️ 为保证测评公平，${type === 'copy' ? '复制' : type === 'cut' ? '剪切' : '粘贴'}功能已被禁用`);
            return false;
        }
    ...
    function activate() {
        ...
        const events = ['keydown', 'keyup', 'keypress', 'contextmenu', 'copy', 'cut', 'paste', 'selectstart', 'dragstart', 'mousedown', 'mouseup'];
