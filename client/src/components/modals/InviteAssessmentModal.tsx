...
const InviteAssessmentModal: React.FC<InviteAssessmentModalProps> = ({ show, onClose, surveyId, surveyTitle }) => {
	...
	return (
		<Modal show={show} title="📧 邀请用户测评" onClose={handleClose}>
			<div className="space-y-4">
				...
				<div>
					<label className="block mb-1 text-gray-700">用户邮箱（可批量，逗号或换行分隔）</label>
					<textarea
				...
				{results.length > 0 && (
					<div className="mt-4">
						...
						<ul className="space-y-1 max-h-40 overflow-y-auto">
							{results.map(r => (
								<li key={r.email} className={r.status === 'success' ? 'text-green-600' : 'text-red-600'}>
									{r.email} - {r.status === 'success' ? '成功' : `失败: ${r.error}`}
								</li>
							))}
						</ul>
					</div>
