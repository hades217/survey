...
export const useAntiCheating = (options: AntiCheatingOptions = {}) => {
  ...
  const preventKeyboardShortcuts = useCallback((e: KeyboardEvent) => {
    ...
    if (disableCopy && (e.ctrlKey || e.metaKey)) {
      if (e.key.toLowerCase() === 'c' || e.key.toLowerCase() === 'x') {
        e.preventDefault();
        e.stopPropagation();
        showWarning(t('survey.antiCheat.copyWarning', 'For fair assessment, copying content is not allowed.'));
        return false;
      }
    }
    ...
    if (disablePaste && (e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'v') {
      e.preventDefault();
      e.stopPropagation();
      showWarning(t('survey.antiCheat.pasteWarning', 'For fair assessment, pasting content is not allowed.'));
      return false;
    }
    ...
    if (disableSelectAll && (e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'a') {
      e.preventDefault();
      e.stopPropagation();
      showWarning(t('survey.antiCheat.selectAllWarning', 'Select all is disabled during assessment.'));
      return false;
    }
    ...
    if (disableDevTools) {
      if (
        e.key === 'F12' ||
        ((e.ctrlKey || e.metaKey) && e.shiftKey && (e.key.toLowerCase() === 'i' || e.key.toLowerCase() === 'j')) ||
        ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'u')
      ) {
        e.preventDefault();
        e.stopPropagation();
        showWarning(t('survey.antiCheat.devToolsWarning', 'Developer tools are disabled during assessment.'));
        return false;
      }
    }
  ...
  useEffect(() => {
    ...
    const contextMenuHandler = (e: MouseEvent) => {
      e.preventDefault();
      e.stopPropagation();
      showWarning(t('survey.antiCheat.rightClickWarning', 'Right-click is disabled during assessment.'));
      return false;
    };
    ...
    if (disableCopy) {
      ...
      document.addEventListener('selectstart', selectStartHandler, { capture: true, passive: false });
    }
  ...
  }, [enabled, disableCopy, disablePaste, disableRightClick, preventKeyboardShortcuts, showWarning, t]);
