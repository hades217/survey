...
export const useAggressiveAntiCheating = (enabled: boolean = true) => {
  useEffect(() => {
    ...
    events.forEach(({ name, handler }) => {
      document.addEventListener(name, handler as EventListener, { capture: true, passive: false });
      document.addEventListener(name, handler as EventListener, { capture: false, passive: false });
      window.addEventListener(name, handler as EventListener, { capture: true, passive: false });
    });
