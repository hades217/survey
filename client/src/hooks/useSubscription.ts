...
export const useSubscription = () => {
	...
	const needsUpgradeFor = useCallback((feature: keyof PlanFeatures): boolean => {
	...
	const getUpgradeMessage = useCallback((feature: keyof PlanFeatures): string => {
	...
	const getRemainingUsage = useCallback((feature: keyof PlanFeatures, currentCount: number): number => {
		if (!currentPlanFeatures) return 0;
		const limit = currentPlanFeatures[feature];
		if (typeof limit === 'number') {
