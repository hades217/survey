...
*Complete coding standards for multilingual (Chinese-English) web applications*
...
export function calculateScore(
  ...
  const correctCount = answers.filter((answer, index) => 
    answer.value === questions[index].correctAnswer
  ).length;
...
*此规范适用于 TypeScript + React 多语言项目，定期更新以适应最新的最佳实践。*