...
router.post('/responses', asyncHandler(async (req, res) => {
	const { name, email, surveyId, answers, timeSpent = 0, isAutoSubmit = false, answerDurations = {} } = req.body;
	...
	const maxPossiblePoints = questionSnapshots.reduce((sum, q) => sum + q.scoring.maxPoints, 0);
