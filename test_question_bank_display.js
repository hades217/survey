...
const TOKEN = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6ImFkbWluIiwidXNlcm5hbWUiOiJhZG1pbiIsInJvbGUiOiJhZG1pbiIsImlhdCI6MTc1MzUwODc5MiwiZXhwIjoxNzU0MTEzNTkyfQ.AhS8CHSFi-KqBmmD7LnHw529UVgdEQnCj9P7Ua1NoxE';
...
async function testQuestionBankDisplay() {
    ...
    try {
        ...
        if (surveysResponse.data) {
            ...
            const questionBankSurveys = surveysResponse.data.filter(survey =>
            ...
            const targetSurvey = surveysResponse.data.find(survey =>
                survey._id === '6884d5228d6ed60daed2391e'
            );
            ...
            if (targetSurvey) {
                ...
                if (targetSurvey.questionBankId) {
                    ...
                    try {
                        const questionBankResponse = await axios.get(`${BASE_URL}/api/admin/question-banks/${targetSurvey.questionBankId}`, {
                            headers: { Authorization: `Bearer ${TOKEN}` }
                        });
                        ...
                        if (questionBankResponse.data) {
                            ...
                            console.log(`   Questions Count: ${questionBankResponse.data.questions?.length || 0}`);
                        }
                    } catch (error) {
                        console.log('❌ Question Bank not found or error:', error.response?.data || error.message);
                    }
                }

                if (targetSurvey.questionBankId && typeof targetSurvey.questionBankId === 'object') {
                    console.log(`   Question Bank: ${targetSurvey.questionBankId.name || 'No name'}`);
                    console.log(`   Question Bank ID: ${targetSurvey.questionBankId._id}`);
                    console.log(`   Question Bank Description: ${targetSurvey.questionBankId.description || 'No description'}`);
                } else {
