...
async function testSurveyTypeRestrictions() {
	...
	try {
		...
		try {
			...
			console.log('   ❌ Failed to create survey with manual source:', error.response?.data?.message || error.message);
		}
		...
		try {
			...
			console.log('   ❌ Failed to create assessment with question bank source:', error.response?.data?.message || error.message);
		}

		console.log('\n🎉 Survey type restrictions test completed!');
		console.log('\nExpected behavior:');
		console.log('- Survey type should only allow manual question creation');
		console.log('- Assessment/Quiz/IQ types should allow both manual and question bank sources');
