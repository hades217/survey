...
function initializeMockData() {
  mockSurveys = [
    { ...sampleQuiz, _id: String(surveyIdCounter++), slug: 'javascript-basics-quiz', createdAt: new Date().toISOString(), isActive: true },
    { ...sampleSurvey, _id: String(surveyIdCounter++), slug: 'user-experience-survey', createdAt: new Date().toISOString(), isActive: true },
    { ...sampleIQTest, _id: String(surveyIdCounter++), slug: 'logic-reasoning-test', createdAt: new Date().toISOString(), isActive: true }
  ];
}
...
async function testStudentFeatures() {
  ...
  const mockResults = [
    { questionText: quiz.questions[0].text, userAnswer: '所有以上', correctAnswer: '所有以上', isCorrect: true, explanation: quiz.questions[0].explanation },
    { questionText: quiz.questions[1].text, userAnswer: ['string', 'number'], correctAnswer: ['string', 'number', 'boolean', 'undefined'], isCorrect: false, explanation: quiz.questions[1].explanation },
    { questionText: quiz.questions[2].text, userAnswer: 'object', correctAnswer: 'object', isCorrect: true, explanation: quiz.questions[2].explanation }
  ];
  
  mockResults.forEach((result, index) => {
    console.log(`\n题目 ${index + 1}: ${result.questionText}`);
    console.log(`您的答案: ${Array.isArray(result.userAnswer) ? result.userAnswer.join(', ') : result.userAnswer}`);
    console.log(`正确答案: ${Array.isArray(result.correctAnswer) ? result.correctAnswer.join(', ') : result.correctAnswer}`);
