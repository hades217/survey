...
async function debugBackendLogin() {
    ...
    try {
        ...
        const registerResponse = await axios.post('http://localhost:8080/api/admin/register', testUser, {
            timeout: 10000,
        ...
        if (registerResponse.status === 201 && registerResponse.data.success) {
            ...
            const loginResponse = await axios.post('http://localhost:8080/api/admin/login', loginData, {
                timeout: 10000,
                validateStatus: () => true // Accept all status codes
            });
    ...
    try {
        ...
        if (healthResponse.status === 401) {
            console.log('✅ Backend is responding (401 is expected for unauthenticated check-auth)'.green);
        } else {
