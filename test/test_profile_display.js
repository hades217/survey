...
async function testProfileDisplay() {
    ...
    if (token) {
        try {
            ...
            if (registerResponse.data.success && registerResponse.data.token) {
                ...
                if (profileResponse.data.user.name === testUser2.name && 
                    ...
                    console.log(`   Actual: ${profileResponse.data.user.name} / ${profileResponse.data.user.email}`.red);
                    failed++;
                }
            } else {
    ...
    console.log('3. Go to Profile Settings (click profile icon or navigate to /admin/profile)'.gray);
