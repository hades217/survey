...
exports.getAllQuestionBanks = async (req, res) => {
	try {
		...
		res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({ error: 'Failed to fetch question banks' });
	}
};
...
exports.getQuestionBank = async (req, res) => {
	try {
		...
		res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({ error: 'Failed to fetch question bank' });
	}
};
...
exports.createQuestionBank = async (req, res) => {
	try {
		...
		if (!name || !name.trim()) {
			return res.status(HTTP_STATUS.BAD_REQUEST).json({ error: 'Question bank name is required' });
		}
		...
		res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({ error: 'Failed to create question bank' });
	}
};
...
exports.updateQuestionBank = async (req, res) => {
	try {
		...
		res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({ error: 'Failed to update question bank' });
	}
};
...
exports.deleteQuestionBank = async (req, res) => {
	try {
		...
		res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({ error: 'Failed to delete question bank' });
	}
};
...
exports.addQuestion = async (req, res) => {
	try {
		...
		if (!options || !Array.isArray(options) || options.length < 2) {
			return res.status(HTTP_STATUS.BAD_REQUEST).json({ error: 'At least 2 options are required' });
		}
		
		if (correctAnswer === undefined || correctAnswer === null) {
			return res.status(HTTP_STATUS.BAD_REQUEST).json({ error: 'Correct answer is required' });
		}
...
exports.updateQuestion = async (req, res) => {
	try {
		...
		if (options !== undefined) question.options = options.map(opt => opt.trim()).filter(opt => opt.length > 0);
...
exports.getRandomQuestions = async (req, res) => {
	try {
		...
		res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({ error: 'Failed to get random questions' });
	}
};
...
exports.importQuestions = async (req, res) => {
	try {
		...
		if (!Array.isArray(questions) || questions.length === 0) {
			return res.status(HTTP_STATUS.BAD_REQUEST).json({ error: 'Questions array is required' });
		}
